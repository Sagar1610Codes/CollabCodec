...
const FileNode = ({ node, path = '' , handleSelectedFile }) => {
        ...
        <div className="ml-4">
          {node.children.map((child, index) => (
            <div key={index} className='ml-0.5'>
              <FileNode key={index} node={child} path={fullPath} handleSelectedFile={handleSelectedFile} />
            </div>
          ))}
        </div>
      )}
...
const FileTree = ({ handleSelectedFile }) => {
  const { projectId } = useParams()
  ...
  return (
    <div className="w-full md:w-[250px] bg-[#1e1e1e] border-blue-400 border-1 text-white p-4 overflow-y-auto h-screen">
      <h4 className="text-sm font-semibold mb-4">Explorer</h4>
      {treeData.map((node, i) => (
        <FileNode key={i} node={node} handleSelectedFile={handleSelectedFile} />
      ))}
    </div>
  )
}
